...
func (r *Responses) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if !hasResponse {
		errs = append(errs, validation.NewValueError(
			...
			fmt.Errorf("responses must contain at least one response code or default"),
			c, c.Default))
	}
...
func (r *Response) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if c.Description.Present && r.Description == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("response.description is required"), c, c.Description))
	}
...
func (h *Header) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if c.Type.Present && h.Type == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("header.type is required"), c, c.Type))
		...
		if !valid {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationAllowedValues, fmt.Errorf("header.type must be one of [string, number, integer, boolean, array]"), c, c.Type))
		}

		// Array type requires items
		if h.Type == "array" && !c.Items.Present {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("header.items is required when type=array"), c, c.Items))
		}
	}
