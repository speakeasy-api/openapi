...
func (s *SecurityScheme) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if c.Type.Present && s.Type == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("securityScheme.type is required"), c, c.Type))
	} else {
	...
	if s.Type == SecuritySchemeTypeAPIKey {
		if !c.Name.Present || s.Name == nil || *s.Name == "" {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("securityScheme.name is required for type=apiKey"), c, c.Name))
		}
		if !c.In.Present || s.In == nil {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("securityScheme.in is required for type=apiKey"), c, c.In))
	...
	if s.Type == SecuritySchemeTypeOAuth2 {
		if !c.Flow.Present || s.Flow == nil {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("securityScheme.flow is required for type=oauth2"), c, c.Flow))
		} else {
		...
		if !c.Scopes.Present {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("securityScheme.scopes is required for type=oauth2"), c, c.Scopes))
		}
	}
