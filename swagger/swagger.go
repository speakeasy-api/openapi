...
func (s *Swagger) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if c.Swagger.Present && s.Swagger == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("swagger is required"), c, c.Swagger))
	} else if c.Swagger.Present && s.Swagger != "2.0" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationSupportedVersion, fmt.Errorf("swagger must be '2.0'"), c, c.Swagger))
	}
	...
	if c.BasePath.Present && s.BasePath != nil && *s.BasePath != "" {
		if !strings.HasPrefix(*s.BasePath, "/") {
			errs = append(errs, validation.NewValueError(
				...
				fmt.Errorf("basePath must start with a leading slash '/'"),
				c, c.BasePath))
		}
	}
