...
func (p *Parameter) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if c.Name.Present && p.Name == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("parameter.name is required"), c, c.Name))
	}

	if c.In.Present && p.In == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("parameter.in is required"), c, c.In))
	...
	if c.AllowEmptyValue.Present && p.In != ParameterInQuery && p.In != ParameterInFormData {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationAllowedValues, fmt.Errorf("parameter.allowEmptyValue is only valid for in=query or in=formData"), c, c.AllowEmptyValue))
	}
		...
		if operation := validation.GetContextObject[Operation](validationOpts); operation != nil {
			...
			if !opCore.Consumes.Present || len(operation.Consumes) == 0 {
				errs = append(errs, validation.NewValueError(
					...
					fmt.Errorf("parameter with type=file requires operation to have consumes defined"),
					c, c.Type))
			} else {
				...
				if !hasValidConsumes {
					errs = append(errs, validation.NewValueError(
						...
						fmt.Errorf("parameter with type=file requires operation consumes to be 'multipart/form-data' or 'application/x-www-form-urlencoded'"),
						c, c.Type))
				}
			}
		}
...
func (p *Parameter) validateParameterType(ctx context.Context, c *core.Parameter, opts ...validation.Option) []error {
	...
	if p.In == ParameterInPath && (!c.Required.Present || !p.GetRequired()) {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("parameter.in=path requires required=true"), c, c.Required))
	}

	// Body parameters require schema
	if p.In == ParameterInBody {
		if !c.Schema.Present {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("parameter.schema is required for in=body"), c, c.Schema))
			return errs
		}
	...
	if !c.Type.Present {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("parameter.type is required for non-body parameters"), c, c.Type))
		return errs
	}
	...
	if p.Type != nil {
		...
		if *p.Type == "file" && p.In != ParameterInFormData {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationAllowedValues, fmt.Errorf("parameter.type=file requires in=formData"), c, c.Type))
		}

		// Array type requires items
		if *p.Type == "array" && !c.Items.Present {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("parameter.items is required when type=array"), c, c.Items))
		}
		...
		if p.CollectionFormat != nil && *p.CollectionFormat == CollectionFormatMulti {
			if p.In != ParameterInQuery && p.In != ParameterInFormData {
				errs = append(errs, validation.NewValueError(
					validation.SeverityError,
					validation.RuleValidationAllowedValues,
					fmt.Errorf("collectionFormat='multi' is only valid for in=query or in=formData"),
...
func (i *Items) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if c.Type.Present && i.Type == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("items.type is required"), c, c.Type))
		...
		if i.Type == "array" && !c.Items.Present {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("items.items is required when type=array"), c, c.Items))
		}
	}
