...
func (i *Info) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if c.Title.Present && i.Title == "" {
		errs = append(errs, validation.NewValueError(validation.NewMissingValueError("info.title is required"), c, c.Title))
	}

	if c.Version.Present && i.Version == "" {
		errs = append(errs, validation.NewValueError(validation.NewMissingValueError("info.version is required"), c, c.Version))
	}

	if c.TermsOfService.Present {
		if _, err := url.Parse(*i.TermsOfService); err != nil {
			errs = append(errs, validation.NewValueError(validation.NewValueValidationError("info.termsOfService is not a valid uri: %s", err), c, c.TermsOfService))
		}
	}
