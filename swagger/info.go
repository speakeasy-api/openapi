...
func (i *Info) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if c.Title.Present && i.Title == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("info.title is required"), c, c.Title))
	}

	if c.Version.Present && i.Version == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("info.version is required"), c, c.Version))
	}
