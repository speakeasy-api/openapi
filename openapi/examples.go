...
func (e *Example) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if core.Value.Present && core.ExternalValue.Present {
		errs = append(errs, validation.NewValueError(validation.NewValueValidationError("value and externalValue are mutually exclusive"), core, core.Value))
	}

	if core.ExternalValue.Present {
		if _, err := url.Parse(*e.ExternalValue); err != nil {
			errs = append(errs, validation.NewValueError(validation.NewValueValidationError(fmt.Sprintf("externalValue is not a valid uri: %s", err)), core, core.ExternalValue))
		}
	}
