...
| <a name="style-description-duplication"></a>`style-description-duplication` | warning | Description and summary fields should not contain identical text within the same object. These fields serve different purposes: summaries provide brief overviews while descriptions offer detailed explanations, so duplicating content provides no additional value to API consumers. |
