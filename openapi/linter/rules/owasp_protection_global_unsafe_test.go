...
func TestOwaspProtectionGlobalUnsafeRule_ValidCases(t *testing.T) {
	t.Parallel()
...
`,
		...
		{
			name: "empty security array allowed",
			yaml: `
openapi: 3.1.0
info:
  title: Test
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rule := &rules.OwaspProtectionGlobalUnsafeRule{}
...
func TestOwaspProtectionGlobalUnsafeRule_Violations(t *testing.T) {
	t.Parallel()
...
func TestOwaspProtectionGlobalUnsafeRule_RuleMetadata(t *testing.T) {
	t.Parallel()

	rule := &rules.OwaspProtectionGlobalUnsafeRule{}

	assert.Equal(t, "owasp-protection-global-unsafe", rule.ID())
