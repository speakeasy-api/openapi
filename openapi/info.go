...
func (i *Info) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if core.Title.Present && i.Title == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("info.title is required"), core, core.Title))
	}

	if core.Version.Present && i.Version == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("info.version is required"), core, core.Version))
	}
...
func (l *License) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if core.Name.Present && l.Name == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("license.name is required"), core, core.Name))
	}
