...
schema:
  type: string
`,
			wantErrs: []string{"[2:1] field in is missing"},
		},
		{
...
schema:
  type: string
`,
			wantErrs: []string{"[4:11] in=path requires required=true"},
		},
		{
...
schema:
  type: string
`,
			wantErrs: []string{"[3:5] in must be one of [query, header, path, cookie]"},
		},
		{
...
`,
			wantErrs: []string{
				...
				"[4:11] in=path requires required=true",
			},
		},
	}
