...
func (p *Parameter) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if core.Name.Present && p.Name == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("parameter.name is required"), core, core.Name))
	}

	if core.In.Present && p.In == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("parameter.in is required"), core, core.In))
	} else {
		switch p.In {
	...
	if p.In == ParameterInPath && (!core.Required.Present || !*p.Required) {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("parameter.in=path requires required=true"), core, core.Required))
	}

	if core.AllowEmptyValue.Present && p.In != ParameterInQuery {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationAllowedValues, fmt.Errorf("parameter.allowEmptyValue is only valid for in=query"), core, core.AllowEmptyValue))
	}

	if core.Style.Present {
		switch p.In {
		case ParameterInQueryString:
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationAllowedValues, fmt.Errorf("parameter field style is not allowed for in=querystring"), core, core.Style))
	...
	if core.Schema.Present {
		switch p.In {
		case ParameterInQueryString:
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationAllowedValues, fmt.Errorf("parameter.schema is not allowed for in=querystring"), core, core.Schema))
		default:
	...
	if !core.Content.Present || p.Content == nil {
		...
		if p.In == ParameterInQueryString {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("parameter.content is required for in=querystring"), core, core.Content))
		}
	} else if p.Content.Len() != 1 {
		// If present, content must have exactly one entry
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationAllowedValues, fmt.Errorf("parameter.content must have exactly one entry"), core, core.Content))
	}
