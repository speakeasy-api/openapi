...
func (r *Reusable[T, V, C]) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	default:
		if r.Value != nil {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationMutuallyExclusiveFields, fmt.Errorf("reusableParameter.value is not allowed when object is not a parameter"), core, core.Value))
		}
	}
