...
func (w *Workflow) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if core.WorkflowID.Present && w.WorkflowID == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("workflow.workflowId is required"), core, core.WorkflowID))
	}
