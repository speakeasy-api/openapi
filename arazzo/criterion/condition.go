...
func (s *Condition) Validate(valueNode *yaml.Node, opts ...validation.Option) []error {
	...
	if s.Expression == "" {
		errs = append(errs, validation.NewValidationError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("expression is required"), valueNode))
	}
	...
	if s.Value == "" {
		errs = append(errs, validation.NewValidationError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("value is required"), valueNode))
	}
