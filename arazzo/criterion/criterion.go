...
func (c *Criterion) Validate(opts ...validation.Option) []error {
	...
	if c.Condition == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("condition is required"), core, core.Condition))
	}
	...
	if c.Type.IsTypeProvided() && c.Context == nil {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("context is required, if type is set"), core, core.Context))
	}
