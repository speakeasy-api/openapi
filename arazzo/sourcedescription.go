...
func (s *SourceDescription) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if core.Name.Present && s.Name == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("sourceDescription.name is required"), core, core.Name))
	}

	if core.URL.Present && s.URL == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("sourceDescription.url is required"), core, core.URL))
