...
func (p *PayloadReplacement) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if core.Target.Present && p.Target == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("payloadReplacement.target is required"), core, core.Target))
	}
	...
	if core.Value.Present && p.Value == nil {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("payloadReplacement.value is required"), core, core.Value))
	} else if p.Value != nil {
