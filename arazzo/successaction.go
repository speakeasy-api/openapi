...
func (s *SuccessAction) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if core.Name.Present && s.Name == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("successAction.name is required"), core, core.Name))
	}

	switch s.Type {
	case SuccessActionTypeEnd:
		if s.WorkflowID != nil {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationMutuallyExclusiveFields, fmt.Errorf("successAction.workflowId is not allowed when type: end is specified"), core, core.WorkflowID))
		}
		if s.StepID != nil {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationMutuallyExclusiveFields, fmt.Errorf("successAction.stepId is not allowed when type: end is specified"), core, core.StepID))
		}
