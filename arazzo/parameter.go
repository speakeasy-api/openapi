...
func (p *Parameter) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if core.Name.Present && p.Name == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("parameter fieldname is required"), core, core.Name))
	}
	...
	default:
		if p.In == nil || in == "" {
			if w == nil && s != nil && s.WorkflowID == nil {
				errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("parameter.in is required within a step when workflowId is not set"), core, core.In))
			}
		}
	...
	if core.Value.Present && p.Value == nil {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("parameter.value is required"), core, core.Value))
	} else if p.Value != nil {
