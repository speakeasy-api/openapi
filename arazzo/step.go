...
func (s *Step) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if core.StepID.Present && s.StepID == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("step.stepId is required"), core, core.StepID))
	} else if s.StepID != "" {
	...
	switch numSet {
	case 0:
		errs = append(errs, validation.NewValidationError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("step at least one of operationId, operationPath or workflowId fields must be set"), core.RootNode))
	case 1:
	default:
		errs = append(errs, validation.NewValidationError(validation.SeverityError, validation.RuleValidationMutuallyExclusiveFields, fmt.Errorf("step only one of operationId, operationPath or workflowId can be set"), core.RootNode))
	}
	...
	if s.OperationID != nil {
		...
		if numOpenAPISourceDescriptions > 1 && !s.OperationID.IsExpression() {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationInvalidSyntax, fmt.Errorf("step.operationId must be a valid expression if there are multiple OpenAPI source descriptions"), core, core.OperationID))
		}
	...
	if s.OperationPath != nil {
		...
		if len(expressionParts) != 1 || expressionParts[0] != "url" {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationInvalidSyntax, fmt.Errorf("step.operationPath must reference the url of a sourceDescription"), core, core.OperationPath))
		}
		if jp == "" {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("step.operationPath must contain a json pointer to the operation path within the sourceDescription"), core, core.OperationPath))
		}
	}
	...
	if s.RequestBody != nil {
		if s.WorkflowID != nil {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationMutuallyExclusiveFields, fmt.Errorf("step.requestBody should not be set when workflowId is set"), core, core.RequestBody))
		}
