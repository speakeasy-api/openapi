...
func (f *FailureAction) Validate(ctx context.Context, opts ...validation.Option) []error {
	...
	if core.Name.Present && f.Name == "" {
		errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationRequiredField, fmt.Errorf("failureAction.name is required"), core, core.Name))
	}

	switch f.Type {
	case FailureActionTypeEnd:
		if f.WorkflowID != nil {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationMutuallyExclusiveFields, fmt.Errorf("failureAction.workflowId is not allowed when type: end is specified"), core, core.WorkflowID))
		}
		if f.StepID != nil {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationMutuallyExclusiveFields, fmt.Errorf("failureAction.stepId is not allowed when type: end is specified"), core, core.StepID))
		}
		if f.RetryAfter != nil {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationMutuallyExclusiveFields, fmt.Errorf("failureAction.retryAfter is not allowed when type: end is specified"), core, core.RetryAfter))
		}
		if f.RetryLimit != nil {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationMutuallyExclusiveFields, fmt.Errorf("failureAction.retryLimit is not allowed when type: end is specified"), core, core.RetryLimit))
		}
	case FailureActionTypeGoto:
		...
		if f.RetryAfter != nil {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationMutuallyExclusiveFields, fmt.Errorf("failureAction.retryAfter is not allowed when type: goto is specified"), core, core.RetryAfter))
		}
		if f.RetryLimit != nil {
			errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationMutuallyExclusiveFields, fmt.Errorf("failureAction.retryLimit is not allowed when type: goto is specified"), core, core.RetryLimit))
		}
	case FailureActionTypeRetry:
		...
		if f.RetryAfter != nil {
			if *f.RetryAfter < 0 {
				errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationAllowedValues, fmt.Errorf("failureAction.retryAfter must be greater than or equal to 0"), core, core.RetryAfter))
			}
		}
		if f.RetryLimit != nil {
			if *f.RetryLimit < 0 {
				errs = append(errs, validation.NewValueError(validation.SeverityError, validation.RuleValidationAllowedValues, fmt.Errorf("failureAction.retryLimit must be greater than or equal to 0"), core, core.RetryLimit))
			}
		}
	default:
