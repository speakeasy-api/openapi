...
`
	...
	t.Logf("Core model embedded map is nil: %v", coreModel.Map == nil)
	if coreModel.Map != nil {
		t.Logf("Core model embedded map length: %d", coreModel.Map.Len())
	}
	...
	require.True(t, highModel.Map.Has("dynamicKey1"))
	require.True(t, highModel.Map.Has("dynamicKey2"))

	// Verify dynamic field values
	dynamicVal1, ok1 := highModel.Map.Get("dynamicKey1")
	require.True(t, ok1)
	...
	dynamicVal2, ok2 := highModel.Map.Get("dynamicKey2")
	require.True(t, ok2)
...
`
	...
	require.NotNil(t, highModel.Map)
	require.Equal(t, 3, highModel.Map.Len())
	require.True(t, highModel.Map.Has("dynamicKey1"))
	require.True(t, highModel.Map.Has("dynamicKey2"))
	require.True(t, highModel.Map.Has("dynamicKey3"))

	// Verify values
	val1, ok1 := highModel.Map.Get("dynamicKey1")
	require.True(t, ok1)
	require.Equal(t, "value1", val1)

	val2, ok2 := highModel.Map.Get("dynamicKey2")
	require.True(t, ok2)
	require.Equal(t, "value2", val2)

	val3, ok3 := highModel.Map.Get("dynamicKey3")
	require.True(t, ok3)
...
`
	...
	postOpHigh, exists := highModel.Map.Get(tests.HTTPMethodPost)
	...
	getOpHigh, exists := highModel.Map.Get(tests.HTTPMethodGet)
	...
	putOpHigh, exists := highModel.Map.Get(tests.HTTPMethodPut)
